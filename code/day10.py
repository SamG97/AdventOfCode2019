...
def find_200th_to_vaporize(grid, station):
    ...
    angles.sort(
        key=lambda pos: (
            (math.atan2(pos[1], pos[0]) + (math.pi / 2)) % (math.pi * 2)
        )
    )
    candidates = asteroids[angles[cycle_fraction - 1]]
    candidates.sort(
        key=lambda pos: (
            abs(pos[0] - station_x) + abs(pos[1]) - station_y
        )
    )
...
if __name__ == "__main__":
    with open("../input/day10.txt") as f:
        g = [[c for c in l.strip("\n")] for l in f]
    s = find_max_observable(g)[1]
    p = find_200th_to_vaporize(g, s)
    print(p[0] * 100 + p[1])
